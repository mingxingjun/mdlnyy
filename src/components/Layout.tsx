...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen bg-[#0e0f1a] font-sans overflow-hidden">
      {/* ── 侧边栏 ── */}
      <aside className="sidebar w-[240px] flex-shrink-0 flex flex-col bg-[#0a0b14] border-r border-white/[0.06]">
        {/* Logo */}
        ...
        <div className="sidebar-nav px-3 pt-4 pb-2 flex-1">
          <p className="sidebar-label px-3 mb-2 text-[11px] font-medium text-[#5c5f73] uppercase tracking-wider">导航</p>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* 页面标题 */}
        <div className="px-4 lg:px-8 pt-5 lg:pt-7 pb-3 flex-shrink-0 bg-[#0a0b14] border-b border-white/[0.06]">
          <h1 className="text-[16px] lg:text-[18px] font-semibold text-[#e8eaf0]">{current.title}</h1>
          <p className="text-[11px] lg:text-[12px] text-[#5c5f73] mt-0.5">{current.subtitle}</p>
        </div>

        {/* 页面内容 */}
        <div className="flex-1 overflow-auto px-4 lg:px-8 py-4 lg:py-6 bg-[#0e0f1a] pb-20 lg:pb-6">
