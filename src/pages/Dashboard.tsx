...
interface ActivityCardConfig {
  view: 'practice' | 'wrongbook' | 'memory' | 'supervisor' | 'questionbank';
  icon: string;
  title: string;
  description: string;
}
...
const ACTIVITY_CARDS: ActivityCardConfig[] = [
  ...
  { view: 'questionbank', icon: '📚', title: '题库管理', description: '查看、编辑、新增题目，按文件筛选练习。' },
...
function ActivityCardList() {
  ...
  const cardMeta: Record<ActivityCardConfig['view'], {
    ...
    questionbank: {
      status: questions.length > 0 ? 'has-reward' : 'inactive',
      count: questions.length,
      countLabel: '题目',
      tagText: questions.length > 0 ? '可管理' : '暂无题目',
      tagColor: questions.length > 0 ? 'seal' : 'worn',
    },
  };
