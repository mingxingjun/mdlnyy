...
const navItems = [
  ...
  { path: '/flow-chamber', icon: Headphones, label: '沉浸流空间', color: 'neon-blue' },
];
...
export default function Layout() {
  return (
    <div className="flex h-screen bg-dark-900 font-body">
      {/* 侧边栏 */}
      <aside className="w-[72px] lg:w-[220px] flex-shrink-0 border-r border-white/5 flex flex-col">
        {/* Logo */}
        <div className="h-16 flex items-center px-4 gap-3 border-b border-white/5">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-neon-blue via-neon-purple to-neon-pink flex items-center justify-center flex-shrink-0">
        ...
        <nav className="flex-1 py-4 px-2 lg:px-3 space-y-1">
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group ${
                  isActive
                    ? `bg-white/5 ${item.color === 'neon-blue' ? 'text-neon-blue' : item.color === 'neon-purple' ? 'text-neon-purple' : 'text-neon-green'}`
            ...
            >
              {({ isActive }) => (
                <>
                  <item.icon size={20} className={isActive ? 'drop-shadow-[0_0_8px_currentColor]' : ''} />
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 flex items-center justify-between px-6 border-b border-white/5 flex-shrink-0">
          <div>
            <h2 className="text-lg font-display font-semibold text-white">期末冲刺周</h2>
            <p className="text-xs text-zinc-500">距离第一门考试还有 8 天</p>
          </div>
